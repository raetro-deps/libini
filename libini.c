...
static struct INI *_ini_open_mem(const char *buf, size_t len, bool free_buf_on_exit) {
...
int ini_read_pair(struct INI *ini, const char **key, size_t *key_len, const char **value, size_t *value_len) {
	size_t _key_len = 0;
	...
	while (true) {
		...
		if(curr == end || *curr == '\n') {
			...
			const char *tmp = curr;
			_key_len = curr - _key;
			for (tmp = curr - 1; tmp > ini->curr && (*tmp == ' ' || *tmp == '\t'); tmp--) {
				_key_len--;
			}
			curr++;
			break;
