...
/**
 * @brief Jump to the next section.
 *
 ...
 * @param name
 * @param name_len
 * @return -EIO if an error occured while reading the file, 0 if no more section can be found, 1 otherwise.
 */
__api int ini_next_section(struct INI *ini, const char **name, size_t *name_len);

/**
 * @brief Read a key/value pair.
 *
 ...
 * @param value
 * @param value_len
 * @return -EIO if an error occured while reading the file, 0 if no more key/value pairs can be found, 1 otherwise.
 */
__api int ini_read_pair(struct INI *ini, const char **key, size_t *key_len, const char **value, size_t *value_len);

/**
 * @brief Set the read head to a specified offset.
 * @param ini
 * @param pointer
 */
...
/**
 * @brief Get the number of the line that contains the specified address.
 * @param ini
 * @param pointer
 * @return -EINVAL if the pointer points outside the INI string, The line number otherwise.
 */
